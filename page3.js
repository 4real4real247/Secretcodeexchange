var RENDERER = {
  ...
  createParticles: function () {
    for (
      var i = 0,
        length = this.TREE_PARTICLE_COUNT,
        hue = 0,
        y = this.BOTTOM_Y,
        theta = 0;
      i < length;
      ...
      var range =
          (0.2 + (0.8 * (this.height / 4 - y)) / (this.height / 2)) *
          this.range,
  ...
  notify: function () {
    for (
      var i = 0, length = this.particles.length, status = this.status;
      i < length;
      i++
    ) {
...
PARTICLE.prototype = {
  ...
  isOutOfSight: function (axis) {
    return (
      (axis.x < -axis.radius && this.vx < 0) ||
      (axis.x > this.renderer.width + axis.radius && this.vx > 0) ||
      (axis.y < -axis.radius && this.vy > 0) ||
      (axis.y > this.renderer.height + axis.radius && this.vy < 0) ||
      (this.z < -this.FOCUS_POSITION && this.vz < 0) ||
      (this.z > this.FOCUS_POSITION && this.vz > 0)
    );
  },
};
...
TREE_PARTICLE.prototype = new PARTICLE({
  ...
  render: function (status, transformCount) {
    ...
    context.fillStyle =
      ...
      (status <= 1
        ? 0
        : (60 * transformCount) / this.renderer.TRANSFORM_COUNT) +
      "%, " +
      (20 + (60 * (this.FOCUS_POSITION - this.z)) / this.renderer.height) +
      "%, " +
      this.opacity +
      ")";
    context.beginPath();
...
CROWN_PARTICLE.prototype = new PARTICLE({
  ...
  init: function () {
    ...
    this.theta = 0;
    this.radius =
      (this.RADIUS * Math.min(this.renderer.width, this.renderer.height)) / 500;
    this.velocity = (this.VELOCITY * this.renderer.height) / 500;
    this.toRender = false;
    this.gradient = this.renderer.context.createRadialGradient(
      0,
      0,
      0,
      0,
  ...
  render: function (status, transformCount) {
    ...
    for (var i = 0, angle = Math.PI / 5; i < 10; i++) {
      ...
      context[i == 0 ? "moveTo" : "lineTo"](
        -length * Math.cos(Math.PI / 2 + angle * i),
        -length * Math.sin(Math.PI / 2 + angle * i)
      );
    }
    context.closePath();
    context.fill();
